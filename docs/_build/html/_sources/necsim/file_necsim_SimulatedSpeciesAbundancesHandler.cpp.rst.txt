

.. _file_necsim_SimulatedSpeciesAbundancesHandler.cpp:

File SimulatedSpeciesAbundancesHandler.cpp
==========================================



Class for repeatedly selecting random species from a distribution of species abundances. 


.. contents:: Contents
   :local:
   :backlinks: none

Definition (``necsim/SimulatedSpeciesAbundancesHandler.cpp``)
-------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_necsim_SimulatedSpeciesAbundancesHandler.cpp.rst



Detailed Description
--------------------

Samuel Thompson
MIT Licence. Contact: samuel.thompson14@imperial.ac.uk or thompsonsed@gmail.com 




Includes
--------


- ``SimulatedSpeciesAbundancesHandler.h`` (:ref:`file_necsim_SimulatedSpeciesAbundancesHandler.h`)





