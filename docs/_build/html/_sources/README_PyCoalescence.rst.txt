...
- Use :py:mod:`setup.py <PyCoalescence.setup>` to customise options and install locations. The procedure is the same
  as exists in :py:func:`main() <PyCoalescence.setup.main>`.
    ...
    .. code-block:: python
        ...
        setup.autoconf() # This may not be necessary and will fail if autotools is not installed.
        setup.run_configure(["--with-hpc", "--with-verbose"]) # Replace with desired install options
        setup.do_compile()
        setup.move_executable("build/custom_install/") # Move to a custom install location
        setup.move_shared_object_file() # Move the .so file to the required destination


- Run ``./configure`` and ``make`` yourself.

    If you require additional compilation options, run ``./configure`` with your options from the NECSim/ directory. Then
    run ``make`` and check that installation is complete. The executables at NECSim/NECSim and NECSim/SpeciationCounter
    should then be moved to where they are required. The shared object file should be moved into the *build/sharedpyx*
    directory, where *x* is the python major version number (2 or 3).
- Custom compilation
...
.. note:: Separate compiles of the program are usually required for each python installation. This can cause
          complications for certain IDEs. If you encounter problems, it is recommended that you run ``setup.py`` from
          within you IDE.
...
.. csv-table::
   ...
   "--with-debug", "Adds additional debugging information"
   "--with-verbose", "Prints information to the terminal. Otherwise, prints to a log file in *Logs/*"
...
.. warning:: It is recommended that ``--with-verbose`` is always used for any HPC system, with the ``--with-hpc`` flag,
...
#. Specify simulation parameters
   ...
   - Optionally run :func:`setup() <PyCoalescence.coalescence.Coalescence.setup>` to link to the correct executables. If
     you have installed via ``python setup.py``, this is not required. Otherwise, specify the path to the executables.
   - Set simulation parameters using :func:`set_simulation_params() <PyCoalescence.coalescence.Coalescence.set_simulation_params>`
   ...
   - Set the map variables by one of the following:
     ...
     a. :func:`set_map_parameters() <PyCoalescence.coalescence.Coalescence.set_map_parameters>` to input file paths and dimensions
     b. :func:`set_map_files() <PyCoalescence.coalescence.Coalescence.set_map_files>` to set the map file paths. This
        calls :func:`detect_map_dimensions() <PyCoalescence.coalescence.Coalescence.detect_map_dimensions>` to automatically
     ...
     .. note:: :func:`detect_map_dimensions() <PyCoalescence.coalescence.Coalescence.detect_map_dimensions>` requires
   ...
   - Optionally, also run :func:`set_speciation_rates() <PyCoalescence.coalescence.Coalescence.set_speciation_rates>`
     to set a list of speciation rates to apply at the end of the simulation.

#. Finalise setup

   - Run :func:`finalise_setup() <PyCoalescence.coalescence.Coalescence.finalise_setup>` to check that simulations are
     setup, create the command to be passed to :ref:`NECSim <Introduction_NECSim>` and create any required config files.

#. Run simulations

   - Finally, start the simulation using :func:`run_coalescence() <PyCoalescence.coalescence.Coalescence.run_coalescence>`
...
:func:`finalise_setup() <PyCoalescence.coalescence.Coalescence.finalise_setup>` to do it for you. This may be useful if
...
#. Add a temporal sampling configuration file
   ...
   - Add temporal sampling points using :func:`add_sample_time() <PyCoalescence.coalescence.Coalescence.add_sample_time>`
     Multiple sample points can be added.
   - Create the temporal sampling config file (:func:`create_temporal_sampling_config() <PyCoalescence.coalescence.Coalescence.create_temporal_sampling_config>`)
#. Generate the main config file
   Run :func:`create_config() <PyCoalescence.coalescence.Coalescence.create_config>` to generate the main config file.

   .. note:: If you wish to use multiple map files or multiple temporal samples and wish to use a main config file as well,
      you must call
      :func:`create_config() <PyCoalescence.coalescence.Coalescence.create_config>` **after** both
      :func:`create_map_config() <PyCoalescence.coalescence.Coalescence.create_map_config>` and
      :func:`create_temporal_sampling_config() <PyCoalescence.coalescence.Coalescence.create_temporal_sampling_config>`
...
#. Add map configuration options
   ...
   - First add the pristine map options using :func:`add_pristine_map() <PyCoalescence.coalescence.Coalescence.add_pristine_map>`
     This can be performed multiple times to add several maps.
   - Add the options to the configuration file (:func:`create_map_config() <PyCoalescence.coalescence.Coalescence.create_map_config>`)
.. note::
...
.. code-block:: python
    ...
    c.set_simulation_params(seed=1, job_type=1, output_directory="output", min_speciation_rate=0.1,
                            sigma=4, tau=4, deme=1, sample_size=0.1, max_time=1)
    # set the speciation rates (this is optional)
...
- Map variables set up using :func:`set_map_parameters() <PyCoalescence.coalescence.Coalescence.set_map_parameters>`
...
- Map files (and variables) set using :func:`set_map_files() <PyCoalescence.coalescence.Coalescence.set_map_files>`
...
:func:`set_simulation_params() <PyCoalescence.coalescence.Coalescence.set_simulation_params>` where *opt* is one of the
following:
...
:func:`optimise_ram() <PyCoalescence.coalescence.Coalescence.optimise_ram>` exists for reducing the RAM requirements of
...
After the function is run, the :class:`Coalescence class <PyCoalescence.coalescence.Coalescence>` should have re-defined
...
:func:`optimise_ram() <PyCoalescence.coalescence.Coalescence.optimise_ram>` may take some time to run. However, for a
...
:func:`get_optimised_solution() <PyCoalescence.coalescence.Coalescence.get_optimised_solution>` and
:func:`set_optimised_solution() <PyCoalescence.coalescence.Coalescence.set_optimised_solution>`. The whole procedure is
outlined below.
...
.. code-block:: python
    ...
    >> c1.optimise_ram()
    # Get the optimised solution for Coalescence object c1
    >> c1.get_optimised_solution()
    {'grid_file_name': 'set',
    'grid_x_size': 5134,
    'grid_y_size': 5134,
    'sample_x_offset': 8208,
    'sample_y_offset': 14877}
    # Now set the optimised solution for Coalescence object c2
    >> c2.set_optimised_solution({'grid_file_name': 'set',
                                  'grid_x_size': 5134,
                                  'grid_y_size': 5134,
                                  'sample_x_offset': 8208,
                                  'sample_y_offset': 14877})
...
Once simulations are complete, the :ref:`SpeciationCounter <Introduction_SpeciationCounter>` program can be used to
apply additional speciation rates to the coalescence tree. A simple way of applying additional simulation rates is
provided within the :class:`Tree class<PyCoalescence.coal_analyse.Tree>`.


The two functions for this routine are

-  :func:`set_speciation_params() <PyCoalescence.coal_analyse.Tree.set_speciation_params>` which takes as arguments
...
-  :func:`apply_speciation() <PyCoalescence.coal_analyse.Tree.apply_speciation>` performs the analysis. This can be extremely
...
.. code-block:: python
    ...
    from PyCoalescence import Tree
    t = Tree()
...
The :class:`Tree class<PyCoalescence.coal_analyse.Tree>` object can also be set up from a
:class:`Coalescence class<PyCoalescence.coalescence.Coalescence>` object as:

.. code-block:: python

    from PyCoalescence import Coalescence, Tree
    sim = Coalescence()
...
- species richness, using :func:`get_richness() <PyCoalescence.coal_analyse.Tree.get_richness>`

- species abundances, using :func:`get_species_abundances() <PyCoalescence.coal_analyse.Tree.get_species_abundances>`

- species octave (2^n) classes for generating species abundance distributions,
  using :func:`get_octaves() <PyCoalescence.coal_analyse.Tree.get_octaves>`
...
.. tip:: The entire list of species can be outputted using
         :func:`get_species_list <PyCoalescence.coal_analyse.Tree.get_species_list>`. This may be useful for scenarios
...
:class:`Tree class <PyCoalescence.coal_analyse.Tree>`.

- :func:`set_database() <PyCoalescence.coal_analyse.Tree.set_database>` generates the link to the SQL database, which
  should be an output from a NECSim simulation (probably run using the
  :class:`Coalescence class <PyCoalescence.coalescence.Coalescence>`.
- :func:`import_comparison_data() <PyCoalescence.coal_analyse.import_comparison_data>` reads an SQL database which
  ...
  - FRAGMENT\_OCTAVES containing *at least* "fragment", "octave", "num_species". This can also be calculated from
    FRAGMENT\_ABUNDANCES using :func:`calculate_comparison_octaves() <PyCoalescence.coal_analyse.Tree.calculate_comparison_octaves>`
...
- The gdal library for both python and C++ (`available here <http://www.gdal.org/>`__). This is **ESSENTIAL** if you wish
  ...
  (using :func:`detect_map_dimensions() <PyCoalescence.coalescence.Coalescence.detect_map_dimensions>`). Both the python
...
- The fast-cpp-csv-parser by Ben Strasser, available
  ...
  *fast-cpp-csv-parser/* should be in the same directory as your C++ header files (the NECSim directory).
