...
.. code-block:: cpp
   ...
   static PyObject * apply(PyObject * self, PyObject * args)
   {
       ...
       PyObject *pItem;
       Py_ssize_t n;
       if (!PyArg_ParseTuple(args, "sisssO!", &database, &record_spatial, &sample_file, &time_config_file,
                             &fragment_file, &PyList_Type, &pList))
       {
   //      PyErr_SetString(PyExc_TypeError, "Speciation rates must be a list.");
           return NULL;
       }
       // Convert all our variables to the relevant form
       string database_str = database;
       bool bSpatial = record_spatial;
       ...
       try
       {
           ApplySpec apply_spec;
           apply_spec.setArgs(database_str, bSpatial, sample_file_str, time_config_file_str, fragment_file_str, spec_rates);
           apply_spec.apply();
       }
       catch(exception &e)
       {
