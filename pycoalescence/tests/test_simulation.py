...
class TestFileCreation(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.coal.set_simulation_parameters(10, 38, "output/test_output/test_output2/", 0.1, 4, 4, deme=1,
                                            sample_size=1.0,
                                            max_time=2,
...
class TestSimulationRaisesErrors(unittest.TestCase):
    ...
    def testProtractedRaisesError(self):
        ...
        c.set_simulation_parameters(6, 4, "output", 0.1, 4, 4, 1, 0.01, 2, dispersal_relative_cost=1,
                                    min_num_species=1, dispersal_method="fat-tail", protracted=True)
        c.set_map_files("null", fine_file="sample/SA_sample_fine.tif",
                        coarse_file="sample/SA_sample_coarse.tif")
        # Now change map name to something that doesn't exist
...
class TestSimulationConfigReadWrite(unittest.TestCase):
    ...
    def testMapConfigWrite(self):
        ...
        """
        self.coal.add_historical_map(fine_file="sample/SA_sample_fine_historical1.tif",
                                     coarse_file="sample/SA_sample_coarse_historical1.tif",
                                     time=1, rate=0.5)
        self.coal.add_historical_map(fine_file="sample/SA_sample_fine_historical2.tif",
        ...
        with open("output/mapconf2.txt", "r") as mapconf:
            ...
            self.assertEqual(lines[22].replace(" ", ""), "path=null",
                             msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[27].replace(" ", ""), "[fine_map]",
                             msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[28].replace(" ", ""), "path=sample/SA_sample_fine.tif",
                             msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[29].replace(" ", ""), "x=13", msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[30].replace(" ", ""), "y=13", msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[31].replace(" ", ""), "x_off=0", msg="Config file doesn't produce expected output.")
            self.assertEqual(lines[32].replace(" ", ""), "y_off=0", msg="Config file doesn't produce expected output.")
    ...
    def testConfigOverwrite(self):
        ...
        with open(config_output, 'a'):
            pass
        sim = Simulation(logging_level=logging.CRITICAL)
        sim.set_simulation_parameters(1, 23, "output", 0.1, 4, 4, 1, 1.0, max_time=200, dispersal_relative_cost=1,
                                      min_num_species=1, dispersal_method="fat-tail", restrict_self=True)
        sim.set_map_files("null", fine_file="sample/SA_sample_fine.tif",
                          coarse_file="sample/SA_sample_coarse.tif")
    ...
    def testConfigWrite(self):
        ...
        coal.set_simulation_parameters(seed=1, job_type=23, output_directory="output", min_speciation_rate=0.1, sigma=4,
                                       tau=4, deme=1, sample_size=1.0, max_time=200, dispersal_relative_cost=1,
                                       min_num_species=1, dispersal_method="fat-tail")
        coal.set_map_files("null", fine_file="sample/SA_sample_fine.tif", coarse_file="sample/SA_sample_coarse.tif")
    ...
    def testConfigWriteErrors(self):
        ...
        coal.set_map_files("null", fine_file="sample/SA_sample_fine.tif", coarse_file="sample/SA_sample_coarse.tif")
        coal.add_historical_map(fine_file="sample/SA_sample_fine_historical1.tif",
                                coarse_file="sample/SA_sample_coarse_historical1.tif", time=1, rate=0.5)
        coal.add_historical_map(fine_file="sample/SA_sample_fine_historical2.tif",
                                coarse_file="sample/SA_sample_coarse_historical2.tif", time=4, rate=0.7)
        coal.rates_list = [0.5]
        with self.assertRaises(ValueError):
    ...
    def testConfigRead(self):
        ...
        self.assertEqual("pycoalescence/tests/sample/SA_sample_fine_pristine1.tif", coal.historical_fine_list[0])
        self.assertEqual("pycoalescence/tests/sample/SA_sample_fine_pristine2.tif", coal.historical_fine_list[1])
        self.assertEqual("pycoalescence/tests/sample/SA_sample_coarse_pristine1.tif", coal.historical_coarse_list[0])
        self.assertEqual("pycoalescence/tests/sample/SA_sample_coarse_pristine2.tif", coal.historical_coarse_list[1])
...
class TestSimulationSetMaps(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.c.set_simulation_parameters(seed=1, job_type=12, output_directory="output", min_speciation_rate=0.01,
                                        sigma=2)
        cls.c.run()
...
class TestSimulationExtremeSpeciation(unittest.TestCase):
    ...
    def testZeroSpeciation(self):
        ...
        c.set_simulation_parameters(seed=1, job_type=17, output_directory="output", min_speciation_rate=0.0,
                                    ...
                                    min_num_species=1, dispersal_method="normal",
    ...
    def testApplySpeciation(self):
        ...
        c = Simulation()
        c.set_simulation_parameters(seed=2, job_type=18, output_directory="output", min_speciation_rate=0.1,
                                    sigma=2.0, tau=1, deme=1, sample_size=1, max_time=4,
                                    dispersal_relative_cost=1,
                                    min_num_species=1, dispersal_method="normal",
                                    landscape_type=False)
...
class TestSimulationMapDensityReading(unittest.TestCase):
    ...
    def testImportingNullMapArrays(self):
        ...
        sim.set_map_files(sample_file=os.path.join("sample", "SA_sample_fine.tif"),
...
class TestHistoricalMapsAlterResult(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.hist_sim.add_historical_map(fine_file="sample/example_historical_fine.tif", coarse_file="none",
                                        time=10, rate=0.2)
        cls.hist_sim.run()
        ...
        cls.hist_sim2.add_historical_map(fine_file="sample/example_historical_fine.tif", coarse_file="none",
                                         time=10, rate=0.2)
        cls.hist_sim2.add_historical_map(fine_file="sample/SA_sample_fine.tif", coarse_file="none",
                                         time=20, rate=0.2)
        cls.hist_sim2.run()
...
class TestSimulationParameters(unittest.TestCase):
    ...
    def testAddingTimes(self):
        ...
        sim = Simulation()
        sim.set_simulation_parameters(seed=10000, job_type=1000000, output_directory="output",
                                      min_speciation_rate=0.1, times=0.1)
        self.assertEqual([0.0, 0.1], sim.times)
        sim = Simulation()
        sim.set_simulation_parameters(seed=10000, job_type=1000000, output_directory="output",
                                      min_speciation_rate=0.1, times=[10, 100])
        self.assertEqual([0.0, 10.0, 100.0], sim.times)
        sim = Simulation()
        sim.set_simulation_parameters(seed=10000, job_type=1000000, output_directory="output",
                                      min_speciation_rate=0.1, times=[100.0])
    ...
    def testSpatialErrors(self):
        ...
        with self.assertRaises(ValueError):
            sim.set_simulation_parameters(seed=10000, job_type=1000000, output_directory="output",
                                          min_speciation_rate=0.1, spatial=False, uses_spatial_sampling=True)
        sim = Simulation()
        with self.assertRaises(ValueError):
            sim.set_simulation_parameters(seed=10000, job_type=1000000, output_directory="output",
                                          min_speciation_rate=0.1, landscape_type="bleh")
...
class TestRamEstimation(unittest.TestCase):
    ...
    def testPersistentRamEstimation(self):
        ...
        sim.set_map_files("null", fine_file=os.path.join("sample", "SA_sample_fine.tif"),
                          coarse_file=os.path.join("sample", "SA_sample_coarse.tif"))
        sim.add_historical_map(fine_file=os.path.join("sample", "SA_sample_fine_pristine1.tif"),
                               coarse_file=os.path.join("sample", "SA_sample_coarse_pristine1.tif"), time=10, rate=0.0)
        self.assertEqual(9689724, sim.persistent_ram_usage())
        sim = Simulation()
        sim.set_simulation_parameters(1, 1000, output_directory="output", min_speciation_rate=0.1)
        sim.set_map_files(sample_file=os.path.join("sample", "SA_sample_coarse.tif"),
...
class TestRamOptimistation(unittest.TestCase):
    ...
    def testBasicOptimisation(self):
        ...
        sim = Simulation()
        sim.set_simulation_parameters(seed=1, job_type=2, output_directory="output",
                                      min_speciation_rate=0.1)
        sim.set_map_files("null", fine_file=os.path.join("sample", "SA_sample_fine.tif"),
                          coarse_file=os.path.join("sample", "SA_sample_coarse.tif"))
        sim.optimise_ram(0.0005)
        self.assertEqual(1, sim.grid.x_size)
        expected_dict = {"grid_x_size": 1,
                         "grid_y_size": 1,
                         "sample_x_offset": 0,
                         "sample_y_offset": 0,
                         "grid_file_name": "set"}
        self.assertEqual(expected_dict, sim.get_optimised_solution())
        expected_dict2 = {"grid_x_size": 10,
                          "grid_y_size": 10,
                          "sample_x_offset": 0,
                          "sample_y_offset": 0,
                          "grid_file_name": "set"}
