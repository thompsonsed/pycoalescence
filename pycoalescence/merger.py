...
class Merger(CoalescenceTree):
    ...
    def __init__(self, database=None, logging_level=logging.WARNING, log_output=None, expected=False):
        """
        Sets up the Merger object.

        :param database: the path to the Merger database object
        :param logging_level: the logging level to assume
        :param log_output: an output stream or file to output logs to
        :param expected: if true, the output database is expected to exist
        """
        super(Merger, self).__init__(logging_level=logging_level, log_output=log_output)
        self.simulation_list = {}
        if database is not None:
            self.set_database(filename=database, expected=expected)
    ...
    def set_database(self, filename, expected=False):
        """
        ...
        :param expected: if true, expects the output to exist

        :rtype: None
        """
        if os.path.exists(filename) and not expected:
            raise IOError("Database already exists at {}".format(filename))
        self._open_database(filename)
        if not expected:
            self._create_simulation_parameters()
            self._create_species_list()
            _ = self.get_added_simulations()
    ...
    def _create_species_abundances(self):
        """
    ...
    def _create_fragment_abundances(self):
        """
    ...
    def _add_species_list(self, species_list, guild):
        ...
        for i, row in enumerate(species_list):
            row[0] = int(elegant_pairing(row[0], guild))
            if i != 0 and row[8] != 0:
    ...
    def _add_species_locations(self, species_locations, guild):
        ...
        for row in species_locations:
            row[0] = elegant_pairing(row[0], guild)
            row.append(guild)
    ...
    def _write_species_locations(self):
        ...
        """
        if self.database is None:
            raise IOError("Cannot insert to SPECIES_LOCATIONS table as the database has not been created.")
        self.cursor = self.database.cursor()
        ...
        try:
            ...
            raise IOError("Could not insert into SPECIES_LOCATIONS table in database {}: {}".format(self.file, soe))
    ...
    def _write_metacommunity_parameters(self):
        ...
        try:
            self.cursor.executemany(create_sql, self.metacommunity_parameters)
            self.database.commit()
        except sqlite3.Error as soe:  # pragma: no cover
            raise IOError(
    ...
    def get_added_simulations(self):
        ...
        :return: dictionary of simulations and guild numbers
        :rtype: dict
        """
        simulations = self.cursor.execute("SELECT filename, guild FROM SIMULATION_PARAMETERS").fetchall()
    ...
    def add_simulation(self, input_simulation):
        ...
        _ = self.get_added_simulations()
        if simulation_path in self.simulation_list.keys():
            raise ValueError("Simulation path of {} has already been added to Merger.".format(simulation_path))
        self.guild += 1
        self.simulation_list[simulation_path] = self.guild
        ...
        if check_sql_table_exist(simulation_path, "SPECIES_LOCATIONS"):
            self._create_species_locations()
            self._add_species_locations(self._read_species_locations(simulation_path), self.guild)
        ...
        if check_sql_table_exist(simulation_path, "FRAGMENT_RICHNESS"):
            self._create_fragment_richness()
    ...
    def generate_guild_tables(self):
        ...
        self.fragment_richness = []
        self.fragment_octaves = []
        for simulation_path, guild in self.simulation_list.items():
            if check_sql_table_exist(simulation_path, "SPECIES_RICHNESS"):
                self._create_species_richness(guilds=True)
                self._add_species_richness(self._read_species_richness(simulation_path), guild)
            if check_sql_table_exist(simulation_path, "FRAGMENT_RICHNESS"):
                self._create_fragment_richness(guilds=True)
                self._add_fragment_richness(self._read_fragment_richness(simulation_path), guild)
            if check_sql_table_exist(simulation_path, "FRAGMENT_OCTAVES"):
                self._create_fragment_octaves(guilds=True)
                self._add_fragment_octaves(self._read_fragment_octaves(simulation_path), guild)
    ...
    def write(self):
        ...
        if len(self.species_locations) > 0:
            self._write_species_locations()
        if len(self.fragment_abundances) > 0:
            self._write_fragment_abundances()
        if len(self.species_richness) > 0:
            self._create_combined_species_richness()
            self._write_species_richness()
        if len(self.fragment_richness) > 0:
            self._create_combined_fragment_richness()
            self._write_fragment_richness()
        if len(self.fragment_octaves) > 0:
            self._create_combined_fragment_octaves()
            self._write_fragment_octaves()
    ...
    def apply(self):
        ...
        """
        self._close_database()
        super(Merger, self).apply()
        self._open_database(self.file)
