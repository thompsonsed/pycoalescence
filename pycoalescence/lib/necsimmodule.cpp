 ...
 * Called in python by necsim.run_from_config("path/to/file")
 * Expects to take either Tree or a child of Tree as its template class.
 * @param self the python object to pass forwards
 * @param args the path to the configuration file
 * @return NULL if error raised, the python return object (as a long) otherwise.
 */
...
/**
 * @brief Template function for resuming the simulation from the given parameters.
 * Expects a Tree or child of Tree as its template class.
 * @param self the python object to call from
 * @param args the arguments to supply to the resume code
 * @return NULL if error raised, the python return object (as a long) otherwise.
 */
